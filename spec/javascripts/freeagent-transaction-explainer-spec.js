  ...
  beforeEach(function() {
    var fixture = $('<div>').attr('id', 'fixture').css('display', 'none')
                  ...
                  .append($('<div>').attr('id', 'attachment_form')
                    .append($('<legend>').text('Attachment')));

    $(document.body).append(fixture);
  });

  it('displays a message when no attachment is required', function() {
    var rules = [{ "textToMatch": "Pact Coffee", "description": "Pact Coffee London", "vat": "0%", "category": "Sundries", "shouldHaveAttachment": false, "ecStatus": "UK/Non-EC" }];
    ...
    var attachmentFromLegend = $('#attachment_form legend');
    expect(attachmentFromLegend.text()).toMatch(/No attachment required/);
  });
});
